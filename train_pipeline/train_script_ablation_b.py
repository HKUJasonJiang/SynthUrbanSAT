...
from scripts.models import SemanticEncoder
...
class AblationBSegOnlyAdapter(nn.Module):
    """(b) Segmentation-only condition baseline."""
    ...
    def forward(self, seg_map, depth_map):
        # Key ablation point: ignore depth condition completely.
        del depth_map
        T_s = self.semantic_encoder(seg_map)
        T_ctrl = self.proj(T_s) * self.output_scale.sigmoid()
...
def parse_args():
    ...
    p.add_argument('--disable-depth', action='store_true',
                   help='Accepted for CLI compatibility; ablation B ignores depth either way.')
    ...
    p.add_argument('--adapter-mlp', action='store_true',
                   help='Accepted for CLI compatibility; ignored by ablation B.')
...
def main():
    ...
    # ── Create Ablation Adapter (b: seg-only) ───────────────────────────────
    phase('[4/8] Creating Ablation Adapter (b: seg-only)', config)
    hdc2a = AblationBSegOnlyAdapter(
        num_classes=config['num_classes'],
        fusion_dim=config['fusion_dim'],
        output_dim=config['control_in_dim'],
        boundary_threshold=config['boundary_threshold'],
    ).to(device, dtype)
    print('  Ablation mode: (b) Segmentation-only condition')
