...
# AblationA does not use SemanticEncoder/DepthEncoder — single unified encoder replaces them
...
from scripts.train import (
    ...
    test_forward_pass, profile_forward_flops,
)
...
class AblationAConcatAdapter(nn.Module):
    """(a) AblationA: ControlNet-native serial framework (串联框架).

    Depth and seg maps are channel-concatenated FIRST, then fed through a
    SINGLE unified ConvStem — no parallel branches, no HDC^2A fusion blocks.
    This mirrors the native ControlNet approach of treating the stacked
    condition channels as one multi-channel control image.

    KEY ABLATION CHANGE vs HDC^2A:
      - HDC^2A: two parallel encoders → cross-attention fusion (并联+融合)
      - AblationA: channel-concat → single shared encoder (串联, one stream)
    """

    def __init__(self, num_classes=5, fusion_dim=768, output_dim=3072,
                 num_fourier_bands=32, boundary_threshold=0.1):
        super().__init__()
        self.num_classes = num_classes
        # AblationA KEY: single ConvStem that takes (num_classes+1) input channels.
        # seg one-hot (num_classes ch) + depth (1 ch) are concatenated before encoding.
    ...
    def forward(self, seg_map, depth_map):
        # AblationA KEY: fuse at input level (channel concat), not at feature level.
        # seg_map: (B, H, W) long  →  one-hot: (B, num_classes, H, W)
        seg_onehot = F.one_hot(seg_map.long(), num_classes=self.num_classes)
        seg_onehot = seg_onehot.permute(0, 3, 1, 2).to(depth_map.dtype)
        # x: (B, num_classes+1, H, W)  — single multi-channel control image
        x = torch.cat([seg_onehot, depth_map], dim=1)

        # Single serial encoder (no parallel branches)
        x = self.conv_stem(x)           # (B, 512, H/8, W/8)
        x = x.flatten(2).transpose(1, 2)  # (B, N, 512)
        x = self.proj(x)                # (B, N, output_dim)
        x = x * self.output_scale.sigmoid()

        B, N, D = x.shape
        H = W = int(math.sqrt(N))
        x = x.transpose(1, 2).reshape(B, D, H, W)
        x = self.spatial_downsample(x)  # (B, D, H/16, W/16)
        x = x.flatten(2).transpose(1, 2)  # (B, N/4, D)
        return x
...
def parse_args():
    ...
    p.add_argument('--disable-depth', action='store_true',
                   help='Zero out depth input before the ablation-A concat adapter.')
    ...
    p.add_argument('--adapter-mlp', action='store_true',
                   help='Accepted for CLI compatibility; ignored by ablation A.')
...
def main():
    ...
    if is_main:
        ...
        _ablation_adapter = 'A: seg+depth channel concat, single encoder'
        _lora_status = 'OFF' if getattr(args, 'no_lora', False) else f'ON (rank={getattr(args, "lora_rank", 32)})'
        _depth_status = 'zeroed by --disable-depth' if getattr(args, 'disable_depth', False) else 'enabled'
    ...
    # ── Create Ablation Adapter (a: seg+depth concat, no fusion) ───────────
    phase('[4/8] Creating Ablation Adapter (a: seg+depth concat, no fusion)', config)
    hdc2a = AblationAConcatAdapter(
    ...
    print('  Ablation mode: (a) Native concat baseline (seg + depth, no HDC^2A fusion)')
