...
def _env_endpoints() -> list[str] | None:
    raw = (os.environ.get("SYNTHURBANSAT_OVERPASS_ENDPOINTS")
           or os.environ.get("PROCEDUREOSM_OVERPASS_ENDPOINTS", "")).strip()
    if not raw:
        return None
...
def _env_timeout(default: int) -> int:
    raw = (os.environ.get("SYNTHURBANSAT_OVERPASS_TIMEOUT")
           or os.environ.get("PROCEDUREOSM_OVERPASS_TIMEOUT", "")).strip()
    if not raw:
        return int(default)
    try:
...
def _post(query: str, endpoint: str, timeout: int = 60) -> dict:
    r = requests.post(endpoint, data={"data": query}, timeout=timeout,
                      headers={"User-Agent": "SynthUrbanSAT-OSM/1.0 "
                                              "(github.com/HKUJasonJiang/SynthUrbanSAT)"})
    r.raise_for_status()
    return r.json()
