"""Batch dataset REGENERATION — no diversity.

For every (rgb, seg, depth) triple in dataset/{train,val} this script
produces ONE RGB output using the unmodified base prompt template plus
the per-image VLM caption — i.e. it just regenerates each source RGB
from its (seg, depth, caption) using the trained ControlNet, with no
diversity injection. Useful for fidelity comparison vs. ground-truth.

Defaults: --n-per-image 1, --dst-root dataset_regenerated/.
Output naming: ``{dst}/{split}/rgb/STEM.tif`` (no __vNN suffix when n=1)
so outputs line up 1:1 with ``dataset/{split}/rgb/STEM.tif`` for direct
comparison.

Usage examples
--------------
Smoke test on 1 val stem:
    python notebooks/1.dataset_regeneration.py --limit 1 --splits val

Full run, both splits:
    python notebooks/1.dataset_regeneration.py
"""
...
def sample_diversity_combos(n: int, rng: random.Random,
    ...
    """Regeneration mode: no diversity injection. Returns *n* empty dicts
    so the prompt template is used as-is (only the VLM caption is added).
    """
...
def _build_prompt_obj(base_template: dict, diversity: Dict[str, str],
    ...
    # otherwise keep the full original template (regeneration mode).
    if has_diversity:
        ...
        for split, items in stems_by_split.items():
            ...
            for stem, rgb_p, seg_p, depth_p in items:
                ...
                for i, var in enumerate(variations):
                    # When n_per_image == 1 use bare stem filename so outputs
                    # align 1:1 with the source dataset for direct comparison.
                    name = stem if n_per_image == 1 else f'{stem}__v{i:02d}'
...
def parse_args():
    ...
    p.add_argument('--dst-root', type=Path,
                   default=PROJECT_ROOT / 'dataset_regenerated',
                   help='output root (will be created)')
    p.add_argument('--splits', nargs='+', default=['train', 'val'],
                   choices=['train', 'val', 'test'])
    p.add_argument('--n-per-image', type=int, default=1,
                   help='regenerations per source stem (default 1)')
